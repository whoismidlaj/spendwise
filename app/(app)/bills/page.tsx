...
export default function BillsPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-3 px-3 py-3 sm:space-y-4 sm:px-4 sm:py-4">
    <Card className="bg-gradient-to-br from-primary to-primary-hover p-4 text-white sm:p-5"><p className="text-sm text-white/75">Monthly recurring bills</p><p className="mt-1 whitespace-nowrap text-3xl font-bold">{formatCurrency(monthlyTotal)}</p><div className="mt-3 truncate border-t border-white/15 pt-3 text-xs text-white/75">{formatCurrency(requiredTotal)} required · {bills.length} active bills</div></Card>
    <Button onClick={() => { setError(''); setEditing(null); setForm(emptyForm()); setFormOpen(true) }} className="w-full gap-2"><Plus size={16} /> Add recurring bill</Button>
