...
export default function IncomePage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-4 px-4 py-4">
    <Card className="bg-gradient-to-br from-emerald-600 to-emerald-700 p-5 text-white"><p className="text-sm text-white/75">Expected in-hand income each month</p><p className="mt-1 text-3xl font-bold">{formatCurrency(totalInHand)}</p><p className="mt-1 text-xs text-white/70">Use expected take-home pay for planning. Gross pay and deductions are kept for reference.</p></Card>
    ...
    <div className="space-y-3">{sources.map(source => {
      ...
      return <Card key={source.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{source.name}</p><p className="text-xs text-gray-500">{source.type.toLowerCase()} · expected on the {source.payday}th · {source.account?.name || 'No account selected'}</p></div><p className="font-bold text-success">{formatCurrency(source.expectedInHand)}</p></div>
        {(source.grossAmount || source.defaultDeductions) && <p className="mt-2 text-xs text-gray-500">Gross: {formatCurrency(source.grossAmount || source.expectedInHand)} · deductions: {formatCurrency(source.defaultDeductions || 0)}</p>}
      <div className="mt-3 flex gap-2 border-t border-border pt-3"><Button id={`receive-income-${source.id}`} size="sm" variant={received ? 'outline' : 'primary'} disabled={received} onClick={() => { setError(''); setReceive(source); setReceivedAmount(String(source.expectedInHand)) }} className="gap-1"> <CheckCircle2 size={14} /> {received ? 'Received this month' : 'Record received'} </Button><Button id={`edit-income-${source.id}`} size="sm" variant="outline" onClick={() => editSource(source)} aria-label={`Edit ${source.name}`}><Edit2 size={14} /></Button><Button id={`delete-income-${source.id}`} size="sm" variant="outline" onClick={() => removeSource(source)} aria-label={`Delete ${source.name}`} className="text-danger"><Trash2 size={14} /></Button></div>
      </Card>
    })}</div>
